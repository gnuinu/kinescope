...
// 전역 히스토그램 이진화. 화면을 정면에서 고르게 찍은 사진에서 기본값보다
// 잘 읽히는 경우가 많습니다.
const READER_OPTS_GLOBAL = { ...READER_OPTS, binarizer: "GlobalHistogram" };
...
// ---- 사진 정밀 스캔 ---------------------------------------------------------
//
// 모니터를 찍은 사진에는 모아레와 JPEG 잡티가 겹쳐서, 촘촘한 QR(버전 25 이상)이
// 여러 장 들어 있으면 한 번에 잘 안 읽힙니다. 4032×3024 사진에 QR 10개가 있으면
// 한 모듈이 3픽셀도 안 됩니다. 그래서 사진은 세 단계로 두드려 봅니다.
//
...
function crop(img, sx, sy, sw, sh) {
  ...
  for (let y = 0; y < sh; y++) {
    ...
    out.set(img.data.subarray(src, src + row), y * row);
  }
  return { data: out, width: sw, height: sh };
}

// 겹치게 자른 타일. 경계에 걸친 QR 이 잘려나가지 않도록 넉넉히 물립니다.
function* tiles(img, n = 2, overlap = 0.12) {
  const tw = img.width / n, th = img.height / n;
  for (let y = 0; y < n; y++) {
    for (let x = 0; x < n; x++) {
      const sx = Math.max(0, Math.round(tw * (x - overlap)));
      const sy = Math.max(0, Math.round(th * (y - overlap)));
      const sw = Math.min(img.width - sx, Math.round(tw * (1 + 2 * overlap)));
      const sh = Math.min(img.height - sy, Math.round(th * (1 + 2 * overlap)));
      if (sw > 32 && sh > 32) yield crop(img, sx, sy, sw, sh);
    }
  }
}

async function scanStill(image) {
  let hit = false;
  const done = () => state().ready;

  // 1) 그대로 — 또렷한 사진은 여기서 끝납니다
  hit = ingest(await readBarcodes(image, READER_OPTS)) || hit;
  if (done()) return hit;

  // 2) 겹치게 자른 타일 — 촘촘한 QR 이 여기서 대부분 살아납니다
  for (const t of tiles(image)) {
    hit = ingest(await readBarcodes(t, READER_OPTS)) || hit;
    if (done()) return hit;
  }

  // 3) 전역 이진화로 한 번 더
  hit = ingest(await readBarcodes(image, READER_OPTS_GLOBAL)) || hit;
  return hit;
}
...
self.onmessage = async (ev) => {
  ...
  try {
    ...
    if (m.type === "frame") {
      framesSeen++;
      // 사진은 시간을 더 들여 정밀하게, 동영상·카메라 프레임은 빠르게.
      const hit = m.deep
        ? await scanStill(m.image)
        : ingest(await readBarcodes(m.image, READER_OPTS));
